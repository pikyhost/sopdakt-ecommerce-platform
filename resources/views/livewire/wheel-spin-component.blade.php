<div class="wheel-container d-flex flex-column align-items-center justify-content-center py-5" style="min-height: 90vh; background: linear-gradient(135deg, #f5f7fa 0%, #e4e8f0 100%);">
    ...
    <div>
        ...
        @if($canSpin)
            ...
            <button class="spin-button btn-glow" id="spinBtn" wire:click="spin">
                <span class="spin-text">إبدأ التدوير</span>
            </button>
        @else
            <div class="cooldown-message text-center">
                @if($wheel->spins_per_user === 1 && $userSpinCount >= 1)
                    <p>لقد استهلكت كل محاولاتك المسموح بها.</p>
                @elseif($cooldownTime)
                    <div class="clock-loader mx-auto mb-3"></div>
                    <p>سيتم تجديد المحاولات خلال:
                        <span class="fw-bold">
                        {{ $cooldownTime->diffForHumans() }}
                    </span>
                    </p>
                @else
                    <p>لا يمكنك اللف حالياً.</p>
                @endif
            </div>
        @endif
