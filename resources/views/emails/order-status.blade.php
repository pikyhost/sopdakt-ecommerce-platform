    ...
    <div class="order-details"
        ...
        @foreach ($order->items as $item)
            <div class="order-item"
                ...
                <!-- Product Image -->
                @if(optional($item->product)->getFeatureProductImageUrl())
                ...
                <div style="flex: 1; text-align: {{ app()->getLocale() == 'ar' ? 'right' : 'left' }}; order: {{ app()->getLocale() == 'ar' ? '2' : '1' }};">
                    ...
                    <p><strong>{{ __('product') }}:</strong> {{ optional($item->product)->getTranslation('name', app()->getLocale()) ?? __('N/A') }}</p>
                    <p><strong>{{ __('quantity') }}:</strong> {{ $item->quantity }}</p>
                    <p><strong>{{ __('price') }}:</strong> ${{ number_format($item->subtotal, 2) }}</p>

                    @if(optional($item->product)->productColors->isNotEmpty())
                        @if($item->size_id)
                            <p><strong>{{ __('size') }}:</strong> {{ optional($item->size)->name ?? __('N/A') }}</p>
                        @endif

                        <p><strong>{{ __('color') }}:</strong>
                            @foreach(optional($item->product)->productColors ?? [] as $productColor)
                                <span class="color-box"
                                      style="background-color:{{ optional($productColor->color)->code ?? '#000' }};">
                        </span> {{ optional($productColor->color)->name ?? __('N/A') }}
                            @endforeach
                        </p>
                    @endif
                </div>
