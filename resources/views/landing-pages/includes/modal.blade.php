...
aria-hidden="true" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
<div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="purchaseModalLabel">{{ __('Checkout') }}</h5>
            <button type="button" class="close {{ app()->getLocale() === 'ar' ? 'close-left' : 'close-right' }}"
        ...
        <div class="modal-body">
            <form  method="post" class="px-3" id="purchaseForm">
                {{-- action="{{route('landing-pages.purchase-form.order',$landingPage->id)}}" --}}
                @csrf
                @method('POST')
                <div class="row">
                    ...
                    <div class="col-md-6 mb-2">
                        <x-select id="governorate" name="governorate_id" label-name="Governorate"
                                onchange="getRegions('governorate', 'region');getShippingCost()" required>
                    ...
                    <div class="col-md-6 mb-2">
                        <x-select id="region" name="region_id" label-name="City" onchange="getShippingCost()">
                        </x-select>
                    </div>
                    @if($landingPage->shippingTypes()->where('landing_page_shipping_types.status',1)->count())
                        <div class="col-md-6 mb-2">
                            <x-select id="shipping_type_id" name="shipping_type_id" label-name="Shipping Type"
                                    onchange="getShippingCost()" required>
                    ...
                    <div class="col-md-6 mb-2">
                        <x-text-area id="notes" name="notes"
                                    label-name="Important note for the company and the representative"></x-text-area>
                    </div>
                </div>
                <div class="row mb-3">
                    <div
                        class="col-12 d-flex justify-content-between align-items-center my-2 border-bottom border border-2 py-2">
                        <h6>{{__('Sub total')}}</h6>
        ...
        <div class="modal-footer" style="direction: {{app()->getLocale() == 'ar' ? 'rtl' : 'ltr'}}">
            ...
            <button type="submit" style="margin-right: 5px;" form="purchaseForm"
                    class="btn btn-primary">{{__('Order')}}</button>
        </div>
    </div>
</div>
</div>
